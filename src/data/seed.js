...
export function getDemoTasks(uid) {
  return [
    ...
    {
      uid,
      name: 'Laporan Praktikum Jaringan Komputer',
    ...
    {
      uid,
      name: 'UTS Software Engineering — Belajar Materi',
    ...
    {
      uid,
      name: 'Quiz Kalkulus — Latihan Soal Integral',
    ...
    {
      uid,
      name: 'Proyek Web Programming — Frontend React',
    ...
    {
      uid,
      name: 'Membaca Paper — Machine Learning Dasar',
      deadline: todayPlus(4),
      difficulty: 2,
      status: 'pending',
      timePref: 'afternoon',
      hours: 2,
    ...
    {
      uid,
      name: 'Olahraga — Renang atau Jogging',
    ...
    {
      uid,
      name: 'Presentasi Kelompok — Sistem Terdistribusi',
      deadline: todayPlus(8),
      difficulty: 3,
      status: 'pending',
      timePref: 'afternoon',
      hours: 3,
    ...
    {
      uid,
      name: 'Kuis Pemrograman Berorientasi Objek',
      deadline: todayMinus(1),
      difficulty: 2,
      status: 'done',
      timePref: 'morning',
      hours: 1,
    ...
    {
      uid,
      name: 'Laporan Kerja Praktek — Minggu 1',
      deadline: todayMinus(3),
      difficulty: 2,
      status: 'done',
      timePref: 'afternoon',
      hours: 2,
...
export async function seedDemoData(uid) {
  ...
  const existingTasks = await getDocs(query(collection(db, 'tasks'), where('uid', '==', uid)));
  existingTasks.docs.forEach(d => batch.delete(d.ref));

  const existingSchedules = await getDocs(query(collection(db, 'schedules'), where('uid', '==', uid)));
  existingSchedules.docs.forEach(d => batch.delete(d.ref));
  const existingSessions = await getDocs(query(collection(db, 'sessions'), where('uid', '==', uid)));
  existingSessions.docs.forEach(d => batch.delete(d.ref));

  await batch.commit();

  const tasks = getDemoTasks(uid);
  for (const task of tasks) {
    await addDoc(collection(db, 'tasks'), task);
  }

  const schedules = getDemoSchedules(uid);
