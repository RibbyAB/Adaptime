...
export default function Schedule({ schedules, setSchedules }) {
  ...
  const save = form => {
    if (modal === 'add') {
      setSchedules(p => [...p, { ...form, id: nid }]);
      setNid(n => n + 1);
    } else {
      setSchedules(p => p.map(s => s.id === modal.id ? { ...s, ...form } : s));
    }
    setModal(null);
  };

  const del = id => setSchedules(p => p.filter(s => s.id !== id));
    ...
    <div className="fade-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 6, marginBottom: 20 }}>
        {DAYS.map(d => (
          ...
          >
            <div style={{ fontSize: 11, fontWeight: 700, color: selDay === d ? '#60A5FA' : '#4B6A8A' }}>
              {d.slice(0, 3)}
            </div>
      ...
      {daySched.length === 0 ? (
        <div className="card" style={{ textAlign: 'center', padding: '50px' }}>
          ...
          <button className="btn btn-ghost" style={{ marginTop: 14 }} onClick={() => setModal('add')}>
            Tambah Jadwal
          </button>
        </div>
      ) : daySched.map(s => (
        ...
        >
          <div style={{ width: 5, background: TYPE_COLORS[s.type], flexShrink: 0 }} />
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <button className="btn btn-ghost" style={{ padding: '5px 11px', fontSize: 12 }} onClick={() => setModal(s)}>
                Edit
              </button>
